        ...
        if(this.props.name == '' && this.props.content == ''){
            return (
                <div className="ts heading centered slate attached">
                    <span className="header lawName">使用方式</span>
                    <span className="description lawContent">
                        ...
                        <p>其中 <code>ㄇㄈ</code> 是民（ㄇㄧㄣˊ）法（ㄈㄚˇ）的注音首字，若同個注音會對應多個法典（如刑法與憲法都是 ㄒㄈ），則會匹配到第一條。</p>
...
class NextButton extends Component {
    render() {
        if(!this.props.available)
            return (
                <div className="ts inverted button disabled">下一條</div>
            );
        else
            return (
                <div className="ts inverted button" onClick={ () =>
            ...
            <Modal show={this.state.helpActive} onClose={() => this.setState({helpActive: false})} updateTime={this.law.getLastUpdate()} />
                <div className="ts centered secondary">
                    <form className="ts form" onSubmit={e => { e.preventDefault(); }}>
                        <div className="field">
                            <input type="text" placeholder="法律名稱（如：刑法 35）" onChange={this.handleChange} value={this.state.value} />
                        </div>
                    </form>
                </div>
